...
export const paymentsApi = {
    ...
    async getAll(year?: number): Promise<Payment[]> {
        const url = year ? `${API_URL}/api/payments?year=${year}` : `${API_URL}/api/payments`;
    ...
    async getAllRaw(year?: number): Promise<Payment[]> {
        const url = year 
            ? `${API_URL}/api/payments?raw=true&includeAllVersions=true&year=${year}` 
            : `${API_URL}/api/payments?raw=true&includeAllVersions=true`;
        const response = await fetch(url, {
            headers: getAuthHeaders(),
        });
